...
如果用C4 model（一种近来流行的架构可视化方法）架构图来可视化这个架构，那么就如图4-2所示。

![图4-2.png](图4-2.png)

图4-2 Promptyoo-0前后端分离架构图（请求流）
...
图4-2展示了Promptyoo-0应用程序的请求流程，展现了从用户到DeepSeek API的完整路径。有助于了解这个架构的核心组成部分。
...
因为Promptyoo原型Web应用需要在本地运行。虽然图4-7显示代码在bolt.new云平台上运行良好，但在本地环境中不一定如此。这是因为bolt.new生成的前端代码依赖于特定版本的JavaScript库。这些代码在bolt.new的云平台上运行正常，因为已经过工程师测试。
...
### 4.4.2 看懂前端代码与/explain和#codebase
...
### 4.4.4 用Inline Chat的/doc为App.tsx加注释
...
### 4.5.3 修复运行错误与Ask子模式下的@terminal
...
我心里默念，前后矛盾的不就是你吗——先是建议用react-scripts test，现在又说这样做不对。不过作为前端开发新手，得罪不起这个活祖宗，我也只能接受它新给出的代码继续尝试了。另外它还让我创建了一个新的配置文件vitest.config.ts，并提供了代码，我也照做。

我在心里嘀咕，自相矛盾的不就是你吗——之前推荐用react-scripts test，现在又说这样不对。但是作为前端开发小白，我也只能乖乖听这位"大佬"的话，接受它最新提供的代码继续往下做了。它还要求我新建一个vitest.config.ts配置文件，并给了相应的代码，我也都照做了。
...
由此可见，使用Copilot进行单元测试的vib编程比编写生产代码时更容易出错。要解决这些问题，需要尝试不同的提示词来找到最有效的解决方案、仔细分析Copilot的回复并做出合理判断，以及在Copilot无法解决问题时，寻求Claude等其他AI编程助手的帮助。
...
与4.6.3类似，后端单元测试写好并运行通过且已验证没有破坏原有的提示词优化功能后，还要验证一下后端单元测试确实起到了保护作用才能放心。
...
要完整保护后端已有代码逻辑，避免未来代码变更造成破坏，还需要像4.6.4节那样使用vibe编程方法生成其他关键的后端单元测试。由于篇幅限制，这部分内容在此略过。感兴趣的读者可以自行尝试实践。
